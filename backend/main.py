...
def calibrate_toxicity(text, prob):
    ...
    toxic_markers = ["kill", "die", "stupid", "idiot", "hate", "abuse", "racist", "nazi"]
    if any(marker in lowered for marker in toxic_markers):
        return float(prob)

    benign_markers = ["naruto", "one piece", "anime", "manga", "movie", "music", "football", "cricket"]
...
def sentiment_label(prob):
    if prob < 0.15: return "Positive / Safe"
    elif prob < 0.35: return "Neutral / Sarcastic"
    else: return "Risky / Negative"

def gemini_prompt(idea, box_count=1):
    box_guidelines = ""
    if box_count == 2:
        box_guidelines = (
            "Split the dilemma into 2 parts. Format exactly as: 'BOX1: [Choice A] | BOX2: [Choice B]'. "
            "Do not include phrases like 'Choose between' or 'Which is better'."
        )
    elif box_count > 2:
        box_guidelines = f"Split the idea into {box_count} distinct parts using 'BOX1:', 'BOX2:', etc. separated by '|'. No other text."
    else:
        box_guidelines = "Keep it a single short, punchy sentence."

    return f"""
You are a master meme creator. Style: Hinglish + modern internet slang.
Text Structure: {box_guidelines}
Rules: High humor, Gen-Z relatable, no generic intros, output ONLY box text.
Idea: {idea}
"""
...
def normalize_caption(text):
    text = re.sub(r'^(Caption|Meme|Text|Output|Idea):\s*', '', text, flags=re.IGNORECASE)
    text = re.sub(r'^choose between\s+', '', text, flags=re.IGNORECASE)
...
def fit_caption_to_boxes(generated_text, box_count):
    generated_text = re.sub(r'^choose between\s+', '', generated_text, flags=re.IGNORECASE)
    ...
    if "|" in generated_text and "BOX1" in generated_text:
        ...
        for i in range(1, box_count + 1):
            ...
            if match: parts.append(match.group(1).strip())
            else: parts.append("")
        if len([p for p in parts if p]) >= min(box_count, 2):
            return parts
    
    if box_count == 2:
        separators = [r"\s+vs\.?\s+", r"\s+or\s+", r"\s*\|\s*", r"\s*/\s*", r"\s+and\s+"]
        for sep in separators:
            parts = re.split(sep, generated_text, flags=re.IGNORECASE)
            if len(parts) == 2:
                p1 = re.sub(r'^BOX1:\s*', '', parts[0], flags=re.IGNORECASE).strip()
                p2 = re.sub(r'^BOX2:\s*', '', parts[1], flags=re.IGNORECASE).strip()
                return [p1, p2]
...
def create_meme(template_id, captions):
    ...
    return requests.post("https://api.imgflip.com/caption_image", data=payload).json()
...
def generate_meme_endpoint(req: MemeRequest):
    ...
    if req.template_id:
        ...
        if not template: template = random.choice(templates[:50])
    else:
